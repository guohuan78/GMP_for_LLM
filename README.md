...
- **扩展测试集**: 9/10 通过 ✓ (90%)
  - 完美匹配: 7 个测试
  - 超越最优: 2 个测试
  - 详细结果见 `TEST_RESULTS.md`

### **测试文件**
- `src/tests.rs` - README示例测试框架
- `src/extended_tests.rs` - 扩展测试运行器
- `test_cases.json` - 10个综合测试用例定义
...
本实现基于 **Bélády最优页面替换算法**，结合内存调度的特定约束进行优化：

### **核心策略**
1. **未来感知卸载**: 卸载时选择"下次使用时间最晚"的内存区域
2. **内存复用**: 检测已加载内存，避免重复Reload
3. **部分卸载**: 只卸载必要的非重叠部分，最小化IO开销
4. **活跃锁定**: Visit期间的内存不可卸载，等待访问完成
5. **Just-in-Time加载**: 在访问前刚好完成加载，最大化并行窗口
6. **并行优化**: RW和Visit操作并行执行，互相掩盖延迟

### **时间复杂度**
- 单次调度决策: O(R log R)，其中 R 为当前HBM中的内存区域数
- 总体: O(N² R log R)，N为请求数量
